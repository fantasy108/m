        ...
        success: function (data) {
            if (data.data.length > 0) {
                ...
                $.each(data.data, function (i, item) {
                    ...
                    var _prolist = $("<li><span class='prolist_img'><img src='" + imgurl + item.firstPhotoUrl + "'></span><h3>" + item.brand + " " + item.vehicleModel + " " + item.selledName + "</h3><p>" + item.currentCity + "<ins>|</ins>" + (new Date(item.registerLicenseDate)).Format("yyyy年M月") + "<ins>|</ins>" + item.displayMileage + "万公里<ins>|</ins>" + item.licenseCodeDisplay + "</p><strong>" + _price + "</strong></li>");
                    ...
                    $("#consultant_list").append(_prolist);
                    power = '0';
                ...
                if (page == 1) {
                    $("#consultant_list").empty();
                    $("#consultant_list").append("<li class='nodata'>抱歉，没有符合条件的顾问</li>");
                }
                power = '1';
            }
            $('#loading').hide();

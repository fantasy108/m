;(function(win,$){
    ...
    sS.prototype.init = function(options){
        ...
        for(var i=0;i<me._liLength;i++){
            $(me.opts.trigger).append('<a class="'+(i==0?me.opts.currCls:"")+'" href="javascript:void(0)"></a>');
        }
        me.triggerBtn=$(me.opts.trigger).find("a");
        ...
        $(win).on('onorientationchange' in win ? 'orientationchange' : 'resize',function(){
            ...
            me.timer = setTimeout(fnGetSlideDistance,80);
        });
    ...
    function fnTouchend(me){
        ...
        if((me._index == 0 && me._moveDistance >0 || (me._index + 1) >= me._liLength && me._moveDistance < 0) && !me.opts.continuousScroll){

            me.goTo(me._index);
            return;
        }
        // 距离小
        if(Math.abs(me._moveDistance) <= me._distance &&  me._endTime-me._startTime>600 ){
            fnSlide(me, '', me.opts.during/1000);
            // 距离大
        }else{
            // 手指触摸上一屏滚动
            if(me._moveDistance >0){
                fnSlide(me, 'prev',  me.opts.during/1000);
                // 手指触摸下一屏滚动
            }else if(Math.abs(me._moveDistance) > 0){
                console.log(me._index);
                fnSlide(me, 'next',  me.opts.during/1000);
            }
        }
        me._moveDistance = me._moveDistanceIE = 0;
    }
    ...
    sS.prototype.goTo = function(i){
        var me = this;
        fnSlide(me, i, me.opts.during/1000);
        me._index=i;
    };
    ...
    function fnSlide(me, go, num){
        ...
        me.triggerBtn.eq(me._index).addClass(me.opts.currCls).siblings().removeClass(me.opts.currCls);
    }

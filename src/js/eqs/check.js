...
$(function(){
    ...
    $('.harm_list,#checked_point').on('click','li',function(){
        ...
        var ds = $(this).attr('data-damagestatusoption')
        //console.log(ds)
        switch(ds){
            case 'damage_status_appearance_1':
                var len = damageStatus_1.length
                var obj = $('.detail_list')
                var html = ''
                for(var i=0;i<len;i++){
                    html += '<li><p>'+damageStatus_1[i]+'</p><i></i></li>'
                }
                obj.empty().append(html)
                break;
            case 'damage_status_appearance_2':
                var len = damageStatus_2.length
                var obj = $('.detail_list')
                var html = ''
                for(var i=0;i<len;i++){
                    html += '<li><p>'+damageStatus_2[i]+'</p><i></i></li>'
                }
                obj.empty().append(html)
                break;
            case 'damage_status_appearance_3':
                var len = damageStatus_3.length
                var obj = $('.detail_list')
                var html = ''
                for(var i=0;i<len;i++){
                    html += '<li><p>'+damageStatus_3[i]+'</p><i></i></li>'
                }
                obj.empty().append(html)
        }
        ...
        $('.detail_list li').each(function(){
            if($(this).children('p').text() == '拆装痕迹'){
                if($(this).hasClass('cur')){
                    $('.damagePhoto').siblings().hide()
                }
            }
        })
...
function isCzhj(){
    ...
    $('.detail_list li').each(function(){
        if($(this).children('p').text() == '拆装痕迹'){
            $(this).hasClass('cur') ? czhj = 1 : czhj = 0
        }else if($(this).children('p').text() != '拆装痕迹'){
            if($(this).hasClass('cur')){
                detail_flag = true;
            }
        }
    })
    var obj = $('.driver_card input,.driver_card label')
    if($('.detail_list').is(':hidden')){
        obj.hide()
    }else if(detail_flag){
        obj.show()
    }else if(detail_flag == false && czhj == 0){
        obj.show()
    }else if(detail_flag == false && czhj == 1){
        obj.hide()
    }
}

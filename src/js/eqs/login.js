/**
 * Created by xuelei.kong on 2015/10/26.
 */
...
var eqs = eqs || {};
eqs.loginPort=siteUrl+"/m/user/login";
function eqs_api() {
    var t = this;
    ...
    if (arguments[0].hasOwnProperty('type')) {
        setting.type = "POST";setting.dataType="json";
    }
    $.ajax(setting);
}
var loginFlag = false;
...
$('input').keyup(function (event) {
    ...
    if ($.trim($('#user_name').val()) == "" || $.trim($('#psw').val()) == "") {
    ...
    if (loginFlag) {
        eqs_api.call(
            eqs.loginPort,
            {
                ...
                password: new Base64().encode($('#psw').val()),
                type: "POST"//post 请求
            },
            function (data) {
                console.log(data);
                if (data.success) {
                    sessionStorage.appraiserId=data.data.id;

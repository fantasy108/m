$(function(){
	...
	if(sessionStorage.shopInfo){
		...
		_info.children('img').attr('src',imgUrl+item.src)
	...
	if(sessionStorage.my_cust_vehicle_info){
		...
		$('.maintain_car').empty().append(html)
	}
	
	/*用户数据*/
	var str5,member,str6,order;
	if(sessionStorage.member_info){
		str5 = sessionStorage.member_info
		member = JSON.parse(str5)
		member.name == null ? $('#name').val('') : $('#name').val(member.name)
		member.cellphone == null ? $('#mobile').val('') : $('#mobile').val(member.cellphone)
	}
	if(sessionStorage.order_info){
		str6 = sessionStorage.order_info
		order = JSON.parse(str6)
		if(order.name != '' || order.name != null || order.name != undefined){
			$('#name').val(order.name)
		}
		if(order.cellphone != '' || order.cellphone != null || order.cellphone != undefined){
			$('#mobile').val(order.cellphone)
		}
		if(order.payment != '' || order.payment != null || order.payment != undefined){
			$('.payment select').val(order.payment)
		}
	}
	...
	/*点击事件*/
	$('.maintain_tit').on('click',function(){
	...
	$('.has_con').on('click',function(){
		if($(this).hasClass('open_icon')){
	        ...
	        sessionStorage.order_info = JSON.stringify(item)
	...
	$('.booking_time').on('click','a',function(){
		var item = {
            name: $('#name').val(),
            cellphone: $('#mobile').val(),
            payment: $('.payment select').val()
